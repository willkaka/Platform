 ...
 */
public class ExExcelUtils {

    /**
     * 单例模式
     * 通过{@link ExExcelUtils#getInstance()}获取对象实例
     */
    private static ExExcelUtils exExcelUtils;

    private ExExcelUtils() {
    }

    public synchronized static ExExcelUtils getInstance() {
        if (exExcelUtils == null) {
            exExcelUtils = new ExExcelUtils();
        }
        return exExcelUtils;
    }
...
//                            .open(inputStream);  //打开资源，必须，可以是InputStream或者是File，注意：只能打开XLSX格式的文件
     ...
     * @param data 待导出数据
     * @param headText 表头字段定义
     * @param sheetName 指定导出Excel的sheet名称
     */
    public void generateSheet(Workbook workbook, List<Map<String,Object>> data, Map<String,String> headText,
                               String sheetName) {
        ...
        if (headText!=null && headText.size()>0 && lastRowNum<=0) {
            // 写标题
...
//            style.setFillForegroundColor(IndexedColors.SKY_BLUE.getIndex());//title单元格背景色
            ...
            for(String key:headText.keySet()){
                row.createCell(i).setCellValue(headText.get(key));
                row.getCell(i).setCellStyle(style);
                headColLengthSumMap.put(i,stringLength(headText.get(key)));
                i++;
            }
        }
        // 写数据
